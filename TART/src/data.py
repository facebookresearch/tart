...
def load_passages(path):
    ...
    with open(path, encoding='utf-8') as fin:
        if path.endswith(".jsonl"):
            ...
            data_repr = io.StringIO(data)  
            reader = csv.reader(data_repr, delimiter="\t")
            for k, row in enumerate(reader):
                if not row[0] == "id":
                    ex = {"id": row[0], "title": row[2], "text": row[1]} if len(row) == 3 else {"id": row[0], "text": row[1]}
                    passages.append(ex)
        if path.endswith(".jsonl") and "_id" in passages[0]:
            for item in passages:
                item["id"] = item["_id"]
                if type(item["text"]) is float:
                    item["text"] = item["title"]
                if type(item["title"]) is float:
                    item["title"] = ""
    return passages
