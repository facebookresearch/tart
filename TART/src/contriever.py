...
import torch
import transformers
from transformers import BertModel, XLMRobertaModel, AlbertModel, T5EncoderModel
...
class ALBERTRetriever(AlbertModel):
    def __init__(self, config, pooling="average", **kwargs):
        super().__init__(config, add_pooling_layer=False)
        if not hasattr(config, "pooling"):
            self.config.pooling = pooling
...
class T5Contriever(T5EncoderModel):
    def __init__(self, config, pooling="average", **kwargs):
...
def load_retriever(model_path, pooling="average", random_init=False):
    ...
    if os.path.exists(path):
        ...
        elif "albert" in retriever_model_id:
            print("Albert Contriever")
            model_class = ALBERTRetriever
        elif "t5" in retriever_model_id or "T0" in retriever_model_id or "gtr" in retriever_model_id:
            model_class = T5Contriever
        else:
